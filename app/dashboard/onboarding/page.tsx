...
const JURISDICTIONS_LIST = [
	{
		code: 'pt',
		name: 'Portugal',
		programs: 'Golden Visa (ARI) · D7 Passive Income',
	},
	{
		code: 'mt',
		name: 'Malta',
		programs: 'MEIN Citizenship · Permanent Residency',
	},
	{ code: 'gr', name: 'Greece', programs: 'Golden Visa (Real Estate) · Fund' },
	{
		code: 'ae',
		name: 'UAE',
		programs: 'Golden Visa (10yr) · Green Card (5yr)',
	},
	{
		code: 'gd',
		name: 'Caribbean',
		programs: 'St Kitts CBI · Antigua CBI · Dominica CBI',
	},
	{ code: 'my', name: 'Malaysia', programs: 'MM2H · Premium Visa Programme' },
	{
		code: 'pa',
		name: 'Panama',
		programs: 'Friendly Nations · Pensionado Visa',
	},
	{
		code: 'us',
		name: 'USA',
		programs: 'EB-5 Investor Visa · E-2 Treaty Investor',
	},
	{
		code: 'sg',
		name: 'Singapore',
		programs: 'Global Investor Programme · ONE Pass',
	},
];
...
const PROGRAMS_BY_JURISDICTION: Record<string, string[]> = {
	...
	Malta: [
		'MEIN Citizenship by Naturalisation',
	...
	return (
		<div
			className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-colors ${
					...
					? 'bg-black text-white'
					: 'bg-gray-100 text-gray-500 border border-[#E5E5E5]'
			}`}
		>
			{done ? '✓' : num}
		</div>
...
function StepCard({
	...
	return (
		...
		>
			...
			{isOpen && (
				<div className='px-5 pb-6 border-t border-[#F0F0F0]'>{children}</div>
			)}
		</div>
	);
}
...
export default function OnboardingPage() {
	...
	const availablePrograms = selectedJurisdictions.flatMap(
		j => PROGRAMS_BY_JURISDICTION[j] ?? [],
	);
			...
			<div className='w-200 mx-auto'>
					...
					>
						<div className='flex items-start gap-4 mb-4'>
							<div
								...
								style={{ background: 'rgba(170,255,69,0.15)' }}
							>
							...
							<div className='flex-1'>
								...
								>
									Some required fields are missing. This ensures your leads are
						...
						<div className='grid grid-cols-4 gap-2'>
							{profileChecks.map(c => (
								...
								>
									<span
										style={{
											color: c.done ? '#AAFF45' : 'rgba(255,255,255,0.3)',
											fontSize: 12,
										}}
									>
										{c.done ? '✓' : '○'}
									...
									<span
										...
										style={{
											color: c.done ? '#AAFF45' : 'rgba(255,255,255,0.35)',
										}}
									>
										{c.label}
									</span>
				...
				<div className='bg-white border border-[#E5E5E5] rounded-xl p-5 mb-5 flex items-center gap-6'>
					<div className='relative shrink-0'>
						...
						<div className='absolute inset-0 flex flex-col items-center justify-center'>
							...
							<span className='text-[9px] text-gray-400 font-semibold tracking-wide mt-0.5'>
								DONE
							</span>
						</div>
					</div>
				...
				<div className='space-y-2.5'>
							...
							profile?.jurisdictions?.some(j =>
								...
								<div className='mt-4 flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-100 rounded-lg'>
									...
									<span className='text-xs text-blue-600 font-medium'>
										Pre-filled from your Settings · verify and save to confirm
									</span>
								</div>
							)}
							...
							<SaveBtn
								...
								onClick={() => {
									if (selectedJurisdictions.length > 0)
										markComplete('jurisdictions', completedSteps);
								}}
							/>
							<span className='text-xs text-gray-400'>
								{selectedJurisdictions.length}/3 selected
							</span>
						</div>
					</StepCard>
						...
						{availablePrograms.length === 0 ? (
							<div className='mt-4 bg-gray-50 rounded-xl p-5 text-center'>
								...
								<div className='text-xs text-gray-400'>
									Select jurisdictions in Step 1 first — programs will appear
									here.
								</div>
							</div>
						) : (
								...
								<div className='flex flex-wrap gap-2'>
									{availablePrograms.map(p => (
										<Chip
											...
											onClick={() =>
												toggle(p, selectedPrograms, setSelectedPrograms)
											}
										/>
									))}
								</div>
							...
							<SaveBtn
								...
								onClick={() => {
									if (selectedPrograms.length > 0)
										markComplete('programs', completedSteps);
								}}
							/>
						</div>
					</StepCard>
					...
					>
						{(profile?.budget ||
							profile?.timeline ||
							(profile?.client_types?.length ?? 0) > 0) && (
							<div className='mt-4 flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-100 rounded-lg'>
								<span className='text-blue-500 text-xs'>↑</span>
								<span className='text-xs text-blue-600 font-medium'>
									Some fields pre-filled from Settings · review and save to
									confirm
								</span>
							</div>
						)}
						<div className='grid grid-cols-2 gap-3 mt-4'>
							<F label='Age Range (min)'>
								<select
									className={sel}
									value={icpAgeMin}
									onChange={e => setIcpAgeMin(e.target.value)}
								>
							...
							<F label='Age Range (max)'>
								<select
									className={sel}
									value={icpAgeMax}
									onChange={e => setIcpAgeMax(e.target.value)}
								>
						...
						<div className='space-y-3 mt-3'>
									...
									<div className='flex flex-wrap gap-2 mt-1'>
										{['Solo', 'Married', 'Family + kids'].map(v => (
											<Chip
												key={v}
												label={v}
												selected={icpFamily.includes(v)}
												onClick={() => toggle(v, icpFamily, setIcpFamily)}
											/>
										))}
									</div>
								</F>
									...
									<div className='flex flex-wrap gap-2 mt-1'>
										{['1–3 mo', '3–6 mo', '6–12 mo', '12+ mo'].map(v => (
											<Chip
												key={v}
												label={v}
												selected={icpTimeline.includes(v)}
												onClick={() => toggle(v, icpTimeline, setIcpTimeline)}
											/>
										))}
									</div>
								</F>
								...
								<div className='flex flex-wrap gap-2 mt-1'>
									{[
										...
										<Chip
											key={v}
											label={v}
											selected={icpMotivation.includes(v)}
											onClick={() => toggle(v, icpMotivation, setIcpMotivation)}
										/>
									))}
								</div>
							</F>
							<F label='Average Deal Value'>
								<select
									className={sel}
									value={icpDeal}
									onChange={e => setIcpDeal(e.target.value)}
								>
						...
						<div className='mt-5'>
							<SaveBtn
								...
								stepId='icp'
								onClick={() => {
									if (icpDeal) markComplete('icp', completedSteps);
								}}
							/>
						</div>
					</StepCard>
					...
					>
						<div className='mt-4 bg-gray-50 rounded-xl p-4 border-l-2 border-black text-xs text-gray-500 leading-relaxed'>
							<strong className='text-gray-900'>Example:</strong> "Our ideal
							client is a 46-year-old business owner from India with $2–5M in
							assets. He wants US green cards for his family within the visa
							timeline — mainly so his children can study and stay in the US,
							and to secure a long-term path to citizenship ."
						</div>
						<div className='space-y-3 mt-4'>
							<F label='Persona Name' hint='A memorable internal reference'>
								<input
									className={inp}
									placeholder='"The Relocating Entrepreneur"'
									value={personaName}
							...
							<F label='Persona Story'>
								<textarea
									...
									rows={4}
									placeholder='Who are they, what do they worry about, what triggered them to look for a second residency?'
									value={personaStory}
									onChange={e => setPersonaStory(e.target.value)}
								/>
							</F>
							<F
								label='Their Biggest Objection'
								hint="What's the main reason they hesitate?"
							>
								<input
									className={inp}
									placeholder={`e.g., "I don't know if Portugal is the right country for us"`}
									value={personaObjection}
									onChange={e => setPersonaObjection(e.target.value)}
								/>
							</F>
							<F label='What Makes Them Say Yes'>
								<input
									className={inp}
									placeholder='"Fast timeline, clear cost breakdown, EU school access for kids"'
									value={personaYes}
									onChange={e => setPersonaYes(e.target.value)}
								/>
							</F>
							<F label='Secondary Persona (optional)'>
								<textarea
									className={`${inp} resize-none`}
									rows={2}
									placeholder='A second type of client, if applicable…'
									value={personaSecondary}
									onChange={e => setPersonaSecondary(e.target.value)}
								/>
							</F>
						</div>
						<div className='mt-5'>
							<SaveBtn
								...
								stepId='persona'
								onClick={() => {
									if (personaStory) markComplete('persona', completedSteps);
								}}
							/>
						</div>
					</StepCard>
					...
					>
						<div className='grid grid-cols-2 gap-3 mt-4'>
							<F
								label='Max new clients / month'
								hint="We'll cap lead volume to match"
							>
								<select
									className={sel}
									value={capacityMax}
							...
							<F
								...
								hint='HOT leads go to fastest responders'
							>
								<select
									className={sel}
									value={capacityResponse}
									onChange={e => setCapacityResponse(e.target.value)}
								>
									<option value=''>Select…</option>
						...
						<div className='mt-3'>
							<F label='Preferred consultation format'>
								<div className='flex flex-wrap gap-2 mt-1'>
									{['Free 20-min Zoom', 'Paid consultation', 'Email first'].map(
										v => (
											<Chip
												key={v}
												label={v}
												selected={capacityFormat.includes(v)}
												onClick={() =>
													toggle(v, capacityFormat, setCapacityFormat)
												}
											/>
										),
									)}
						...
						<div className='mt-5'>
							<SaveBtn
								...
								stepId='capacity'
								onClick={() => {
									if (capacityMax && capacityResponse)
										markComplete('capacity', completedSteps);
								}}
							/>
						</div>
					</StepCard>
