...
export default function DashboardPage() {
	...
	const profileComplete = !!profile?.company_name && !!user?.email && !!profile?.full_name && !!profile?.website;
	...
	const removeForm = async (formId: string) => {
		if (!user?.id) return;
		try {
			await supabase.from('partner_forms').delete().eq('partner_id', user.id).eq('form_id', formId);
			await fetchUser();
		} catch {}
	};
	...
	const setupSteps = [
		...
		},
		{
			...
			desc: 'Connect your form ID with a UTM label to start routing leads into your dashboard.',
			href: '#integration',
			done: forms.length > 0,
			cta: forms.length > 0 ? 'Manage below' : 'Add below',
		},
		{
			num: '03',
	...
	return (
		<div className='min-h-screen bg-[#fafafa] px-8 py-7'>
			<div style={{ maxWidth: 780 }}>
				...
				<div className='grid grid-cols-3 gap-3 mb-5'>
					{[
						...
						},
						{
							...
							value: `${obPct}%`,
							sub: obPct === 100 ? 'Fully configured' : `${TOTAL_STEPS - Math.round(obPct / 20)} steps remaining`,
							accent: obPct === 100,
						},
						{
							label: 'Integrations',
							value: forms.length > 0 ? String(forms.length) : '—',
							sub: forms.length > 0 ? `form${forms.length > 1 ? 's' : ''} connected` : 'Not connected',
							accent: forms.length > 0,
						},
					].map(s => (
						<div key={s.label} className='bg-white border border-[#E5E5E5] rounded-xl px-4 py-3.5'>
							<p className='text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-1.5'>{s.label}</p>
							<p className={`text-2xl font-extrabold leading-none ${s.accent ? 'text-gray-900' : 'text-gray-300'}`}>
								{s.value}
							</p>
				...
				{/* Setup steps */}
				<div className='mb-5'>
					<p className='text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-3'>Setup Checklist</p>
					<div className='grid grid-cols-3 gap-3'>
						{setupSteps.map(s => (
							<div
								key={s.num}
								className={`bg-white rounded-xl border p-4 flex flex-col transition-all ${
									s.done ? 'border-green-200 bg-green-50/30' : 'border-[#E5E5E5]'
								}`}
							>
								<div className='flex items-center justify-between mb-3'>
									<span className='text-xs font-black text-gray-300'>{s.num}</span>
									<span className={`w-2 h-2 rounded-full shrink-0 ${s.done ? 'bg-green-400' : 'bg-amber-300'}`} />
								</div>
								<p className='text-sm font-bold text-gray-900 mb-1.5'>{s.title}</p>
								<p className='text-[11px] text-gray-400 leading-relaxed flex-1'>{s.desc}</p>
								<Link
									href={s.href}
									className={`mt-3 text-xs font-semibold transition-colors ${
										s.done ? 'text-green-600 hover:text-green-700' : 'text-black hover:text-gray-500'
									}`}
								>
									{s.cta} →
								</Link>
				...
				{/* Integration */}
				<div id='integration' className='bg-white border border-[#E5E5E5] rounded-xl overflow-hidden'>
					<div className='px-5 py-4 border-b border-[#F0F0F0]'>
						<div className='flex items-center justify-between mb-3'>
							<h2 className='text-sm font-bold text-gray-900'>Form Integrations</h2>
							{forms.length > 0 && (
						...
						<div className='flex items-start gap-2.5 px-3 py-2.5 bg-blue-50 border border-blue-100 rounded-lg'>
							...
							<p className='text-xs text-blue-700 leading-relaxed'>
								...
								<strong className='font-semibold'>Contact your administrator</strong> — they will provide you with the credentials needed to connect your lead source.
							</p>
						</div>
					</div>
					...
					{forms.length > 0 && (
						<div className='divide-y divide-[#F5F5F5]'>
							{forms.map(f => (
								<div key={f.form_id} className='flex items-center justify-between px-5 py-3'>
									<code className='text-[11px] text-gray-400'>
										/api/leads/form/<span className='text-gray-900 font-semibold'>{f.form_id}</span>
										<span>?utm_content=</span>
										<span className='text-gray-900 font-semibold'>{f.utm_content}</span>
									</code>
									<button
					...
					<div className='px-5 py-4 bg-[#fafafa] border-t border-[#F0F0F0]'>
						<div className='grid grid-cols-2 gap-2 mb-2'>
							<div>
								<label className='text-[10px] font-bold uppercase tracking-wider block mb-1 text-gray-400'>Form ID</label>
								<input className={inp} placeholder='e.g. xX4jrr' value={newFormId} onChange={e => setNewFormId(e.target.value)} />
							</div>
							<div>
								<label className='text-[10px] font-bold uppercase tracking-wider block mb-1 text-gray-400'>utm_content</label>
								<input className={inp} placeholder='e.g. facebook_ad' value={newUtmContent} onChange={e => setNewUtmContent(e.target.value)} onKeyDown={e => e.key === 'Enter' && addForm()} />
							</div>
						</div>
						{newFormId.trim() && newUtmContent.trim() && (
							<p className='text-[10px] text-gray-400 font-mono mb-2'>
								→ /api/leads/form/{newFormId.trim()}?utm_content={newUtmContent.trim()}
							</p>
						)}
